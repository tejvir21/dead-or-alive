...
export default function GamePage() {
  ...
  useEffect(() => {
    ...
    const onPlayerSkipped = ({ username }) => {
      useGameStore
        .getState()
        .showNotification?.(`${username} is ready to choose`, "info");
    };
    ...
    // Unity co-op: notify when the team's shared choice has been made
    const onTeamChoiceMade = ({ by, teamId }) => {
      useGameStore
        .getState()
        .showNotification?.(`${by} chose for the team!`, "info");
      useGameStore.setState({ chosenCount: 1, totalAlive: 1 }); // team choice made = "done"
    };

    // Vs mode: a team was fully eliminated
    const onTeamEliminated = ({ teamId, teamName, atRoom }) => {
      useGameStore
        .getState()
        .showNotification?.(
          `💀 ${teamName || "Team " + teamId} was eliminated at room ${atRoom}!`,
          "error",
        );
    };
    ...
    const onTimerReduced = ({ reduction, by }) => {
      ...
      useGameStore
        .getState()
        .showNotification?.(
          `⚡ ${by} chose early — ${reduction}s removed!`,
          "warning",
        );
    };
    ...
    const onPlayerEliminated = ({ username }) => {
      useGameStore
        .getState()
        .showNotification?.(`💀 ${username} was eliminated!`, "error");
    };
    ...
    const onYouWereKicked = ({ reason }) => {
      setKickedInfo({ reason });
    };
    const onPlayerKicked = ({ session: s }) => {
      if (s) useGameStore.getState().updateSession?.(s);
    };
    ...
    socket.on("teamChoiceMade", onTeamChoiceMade);
    socket.on("teamEliminated", onTeamEliminated);
    ...
    return () => {
      ...
      socket.off("teamChoiceMade", onTeamChoiceMade);
      socket.off("teamEliminated", onTeamEliminated);
  ...
  const handleSpectate = () =>
    connectSocket()?.emit("spectateAfterElimination", { roomCode: code });
  ...
  if (roundPhase === "game_end" && gameEndData) {
    const handleLeaveCompletedGame = () => {
      useGameStore.setState({
        roundPhase: "idle",
        gameEndData: null,
        currentRoom: null,
    ...
    return (
      <GameEndScreen
        data={gameEndData}
        player={player}
        onLobby={handleLeaveCompletedGame}
      />
    );
  ...
  const myTeamId = players.find((p) => p.username === player?.username)?.teamId;
  const coopSubMode = session?.coopSubMode;
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white flex flex-col">
      ...
      {/* Team-only chat — floating panel, co-op/vs only */}
      {isTeamMode && !isSpectating && myTeamId && (
        <TeamChat
          roomCode={code}
          teamId={myTeamId}
      ...
      {/* Vs-mode team status bar */}
      {mode === "vs" && session?.teams && (
        <TeamScoreboard teams={session.teams} myTeamId={myTeamId} />
      )}

      {mode === "coop" && (
        <div className="bg-cyan-950/30 border-b border-cyan-900 px-4 py-1.5 text-center">
          <p className="font-mono text-[10px] text-cyan-400">
            🤝 CO-OP ·{" "}
            {coopSubMode === "unity"
              ? "UNITY — one door for the whole team"
              : "RESILIENCE — team survives if anyone does"}
          </p>
        </div>
      )}
      ...
      {isSpectating && (
        <div className="bg-purple-950/50 border-b border-purple-800 px-4 py-2 text-center">
          <p className="font-mono text-xs text-purple-300">
            👁 SPECTATING — you were eliminated. Clue details are hidden.
          </p>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col">
        <AnimatePresence mode="wait">
          {(loading ||
            (!currentRoom &&
              !["reveal", "game_end", "transition"].includes(roundPhase))) && (
            <motion.div
              key="loading"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
