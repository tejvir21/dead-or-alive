...
router.post('/players/:id/ban', protect, adminOnly, async (req, res) => {
  try {
    ...
    await AuditLog.create({ adminId: req.player._id, adminName: req.player.username, action: 'player.ban', target: player.username, details: { reason, durationDays } });
...
router.post('/players/:id/unban', protect, adminOnly, async (req, res) => {
  try {
    ...
    await AuditLog.create({ adminId: req.player._id, adminName: req.player.username, action: 'player.unban', target: player.username });

    // ADD THIS:
    const io = req.app.get('io');
    notify(io, player._id, {
      type: 'admin_unbanned', title: 'Account Unbanned',
  ...
  try {
    ...
    if (req.body.isVerified !== undefined) {
      ...
      notify(io, player._id, {
        ...
        title: req.body.isVerified ? 'Account Verified' : 'Verification Removed',
...
```js
router.post('/players/:id/subscription', protect, adminOnly, async (req, res) => {
  try {
    const { plan, durationDays } = req.body;
    const expiresAt = new Date(Date.now() + durationDays * 86400000);
    const player = await Player.findByIdAndUpdate(req.params.id, {
      'subscription.plan': plan, 'subscription.status': 'active', 'subscription.expiresAt': expiresAt,
    }, { new: true });

    await AuditLog.create({ adminId: req.player._id, adminName: req.player.username, action: 'player.giftSubscription', target: player.username, details: { plan, durationDays } });
